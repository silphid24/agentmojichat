...
class RAGHealthChecker:
    ...
    async def check_directories(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if docs_dir.exists():
                ...
                self.print_check("문서 디렉토리", "PASS", f"{docs_dir} ({doc_count}개 파일)")
            ...
            if vectordb_dir.exists():
                ...
                self.print_check("벡터 DB 디렉토리", "PASS", f"{vectordb_dir} ({db_size/1024/1024:.1f} MB)")
                checks['vectordb_dir'] = {'exists': True, 'size_mb': db_size/1024/1024}
            else:
                self.print_check("벡터 DB 디렉토리", "WARN", f"{vectordb_dir} 존재하지 않음")
            ...
            if metadata_file.exists():
                ...
                self.print_check("메타데이터 파일", "PASS", f"{len(metadata)}개 파일 추적 중")
    ...
    async def check_vector_store(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.print_check("청크 설정", "PASS", f"크기: {chunk_size}, 중복: {chunk_overlap}")
        ...
        try:
            ...
            for query in test_queries:
                ...
                try:
                    ...
                    self.print_check(f"검색: '{query[:20]}...'", "FAIL", f"오류: {str(e)}")
                    performance_results.append({
                        'query': query,
                        'error': str(e),
                        'status': 'FAIL'
        ...
        try:
            ...
            if result and result.get('answer'):
                ...
                if 'search_metadata' in result:
                    ...
                    self.print_check("쿼리 재작성", "PASS", f"{rewritten_queries}개 변형")
    ...
    async def generate_recommendations(self, all_results: Dict[str, Any]) -> List[str]:
        ...
        if not env_checks.get('api_key'):
            recommendations.append("🔑 OpenAI API 키를 설정하세요 (.env 파일의 OPENAI_API_KEY)")
        
        if env_checks.get('packages', {}).get('missing'):
            missing = env_checks['packages']['missing']
            recommendations.append(f"📦 누락된 패키지를 설치하세요: pip install {' '.join(missing)}")
        
        # 디렉토리 문제
        dir_checks = all_results.get('directories', {})
        if not dir_checks.get('docs_dir', {}).get('exists'):
            recommendations.append("📁 문서 디렉토리를 생성하고 문서를 추가하세요: mkdir -p data/documents")
        
        if dir_checks.get('docs_dir', {}).get('file_count', 0) == 0:
            recommendations.append("📄 data/documents/ 폴더에 문서를 추가한 후 python upload_docs.py를 실행하세요")
        ...
        if avg_time and avg_time > 2.0:
            recommendations.append("⚡ 검색 성능이 느립니다. 청크 크기를 줄이거나 문서를 정리하세요")
        
        # RAG 파이프라인 문제
        rag_checks = all_results.get('rag_pipeline', {})
        if not rag_checks.get('query_successful'):
            recommendations.append("🔧 RAG 파이프라인에 문제가 있습니다. 로그를 확인하세요")
        
        # LLM 통합 문제
        llm_checks = all_results.get('llm_integration', {})
        if not llm_checks.get('langchain_model'):
            recommendations.append("🤖 LLM 통합에 문제가 있습니다. API 키와 프로바이더 설정을 확인하세요")
    ...
    async def run_full_check(self, verbose: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if verbose:
                ...
                print(f"📊 전체 점검: {passed_checks}/{total_checks} 통과 ({success_rate:.1f}%)")
