...
def mock_auth(monkeypatch):
    ...
    mock_user = "test_user"
    monkeypatch.setattr(
        "app.api.v1.endpoints.vectorstore.get_current_user", lambda: mock_user
    )
    return mock_user
...
class TestVectorStoreAPI:
    ...
    def test_create_store(self, client, auth_headers, mock_auth):
        ...
        with patch(
            "app.api.v1.endpoints.vectorstore.vector_store_manager"
        ) as mock_manager:
            mock_store = Mock()
            mock_store.get_collection_stats = AsyncMock(
                return_value={"store_type": "chroma", "document_count": 0}
            )
    ...
    def test_list_stores(self, client, auth_headers, mock_auth):
        ...
        with patch(
            ...
            response = client.get("/api/v1/vectorstore/stores", headers=auth_headers)
    ...
    def test_hybrid_search(self, client, auth_headers, mock_auth):
        ...
        with patch(
            ...
            mock_store.hybrid_search.assert_called_once_with(
                query="test query", k=5, alpha=0.7, filter=None
            )
    ...
    def test_upload_and_index_file(self, client, auth_headers, mock_auth):
        ...
        with patch(
            ...
            with patch(
                "app.api.v1.endpoints.vectorstore.DocumentProcessor"
            ) as mock_processor:
                mock_processor_instance = Mock()
                mock_processor_instance.process_file = AsyncMock(
                    return_value=[
    ...
    def test_clear_store(self, client, auth_headers, mock_auth):
        ...
        with patch(
            ...
            response = client.post(
                "/api/v1/vectorstore/stores/test_store/clear", headers=auth_headers
            )
    ...
    def test_optimize_stores(self, client, auth_headers, mock_auth):
        ...
        with patch(
            ...
            mock_manager.optimize_stores = AsyncMock(
                return_value={"store1": True, "store2": True, "store3": False}
            )

            response = client.post(
                "/api/v1/vectorstore/stores/optimize", headers=auth_headers
            )

            assert response.status_code == 200
            data = response.json()
            assert data["message"] == "Optimization completed"
            assert data["results"]["store1"] is True
            assert data["results"]["store3"] is False
    ...
    def test_migrate_data(self, client, auth_headers, mock_auth):
        ...
        with patch(
            ...
            response = client.post(
                ...
                params={"from_store_id": "store1", "to_store_id": "store2"},
                headers=auth_headers,
            )
            ...
            mock_manager.migrate_data.assert_called_once_with(
                from_store_id="store1", to_store_id="store2"
            )
    ...
    def test_search_all_stores(self, client, auth_headers, mock_auth):
        ...
        with patch(
            ...
            mock_manager.search_all_stores = AsyncMock(
                return_value={
                    "store1": [
                        SearchResult(
            ...
            response = client.post(
                ...
                json={"query": "search all stores", "k": 3},
                headers=auth_headers,
            )
    ...
    def test_delete_store(self, client, auth_headers, mock_auth):
        ...
        with patch(
            ...
            response = client.delete(
                "/api/v1/vectorstore/stores/test_store", headers=auth_headers
            )
