...
async def test_parallel_vs_sequential():
    ...
    try:
        ...
        print(f"  - 각 쿼리는 여러 변형으로 재작성됨")
        print(f"  - 병렬 vs 순차 처리 비교")
        print()
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            try:
                # 병렬 검색 비활성화
                sequential_docs, sequential_meta = await rag_pipeline.search_with_rewriting(
                ...
                print(f"   🔄 재작성 쿼리: {len(sequential_meta.get('rewritten_queries', []))}개")
    ...
    try:
        ...
        if single_time > 0:
            ...
            if overhead_ratio < 2.0:
                print(f"   ✅ 쿼리 확장 대비 오버헤드 합리적")
            elif overhead_ratio < 3.0:
                print(f"   🔄 쿼리 확장 대비 오버헤드 보통")
            else:
                print(f"   ⚠️  쿼리 확장 대비 오버헤드 높음")
        ...
        if len(parallel_docs) > len(single_docs):
            print(f"   ✅ 다중 쿼리로 더 많은 관련 문서 발견")
        elif len(parallel_docs) == len(single_docs):
            print(f"   🔄 결과 수는 동일, 품질 개선 가능성")
        else:
            print(f"   ⚠️  다중 쿼리로 결과 수 감소")
...
if __name__ == "__main__":
    async def main():
        ...
        print(f"\n🎉 모든 비동기 성능 테스트 완료!")
        print(f"💡 병렬 처리로 검색 성능이 크게 향상되었습니다!")
