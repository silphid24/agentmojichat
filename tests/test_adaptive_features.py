...
async def test_query_complexity_analysis():
    ...
    for i, test_case in enumerate(test_queries, 1):
        ...
        print(
            f"   활성화 기능: {', '.join(active_features) if active_features else '기본만'}"
        )
    ...
    print(
        f"📈 복잡도 예측 정확도: {accuracy:.1f}% ({correct_predictions}/{len(test_queries)})"
    )
    ...
    try:
        ...
        for i, scenario in enumerate(test_scenarios, 1):
            ...
            try:
                ...
                if adaptive_config:
                    print(
                        f"   🧠 처리 모드: {adaptive_config.get('processing_mode', 'unknown')}"
                    )
                    print(
                        f"   📊 복잡도: {adaptive_config.get('complexity', 'unknown')}"
                    )
                    ...
                    actual = elapsed_time
                    prediction_error = (
                        abs(estimated - actual) / max(estimated, 0.1) * 100
                    )
                    print(
                        f"   📈 시간 예측: {estimated:.1f}초 예상, {actual:.1f}초 실제 (오차: {prediction_error:.1f}%)"
                    )
        ...
        if perf_stats["count"] > 0:
            ...
            print(
                f"   최소/최대: {perf_stats['min']:.3f}초 / {perf_stats['max']:.3f}초"
            )
    ...
    try:
        ...
        for i, test_case in enumerate(feature_tests, 1):
            ...
            for feature_name, expected_state in expected.items():
                actual_state = config.get(
                    f"use_{feature_name}", config.get(feature_name, False)
                )

                status = "✅" if actual_state == expected_state else "❌"
                print(
                    f"   {status} {feature_name}: {actual_state} (예상: {expected_state})"
                )
...
async def test_processing_modes():
    ...
    try:
        ...
        for i, test_case in enumerate(mode_tests, 1):
            ...
            print(
                f"   {status} 처리 모드: {actual_mode.value} (예상: {expected_mode.value})"
            )
            print(f"   📊 추정 시간: {analysis.estimated_processing_time:.1f}초")
            print()
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        if accuracy >= 70:
            print(
                "💡 적응형 시스템이 효과적으로 작동하여 쿼리별 최적화된 처리가 가능합니다!"
            )
        else:
            print("⚠️  적응형 시스템 개선이 필요하지만, 기본 기능은 정상 작동합니다.")
