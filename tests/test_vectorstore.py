...
class TestChromaVectorStore:
    ...
    @pytest.mark.asyncio
    async def test_add_documents(self, vector_config, sample_documents, mock_embeddings):
    ...
    async def test_delete_documents(self, vector_config, mock_embeddings):
        ...
        with patch('chromadb.Client') as mock_client:
            ...
            with patch('langchain.vectorstores.Chroma') as mock_langchain_chroma:
                ...
                assert success
                mock_collection.delete.assert_called_once_with(where={"source": "test.txt"})
    ...
    async def test_hybrid_search(self, vector_config, mock_embeddings):
        ...
        with patch('chromadb.Client') as mock_client:
            ...
            with patch('langchain.vectorstores.Chroma') as mock_langchain_chroma:
                mock_lc_instance = Mock()
                mock_lc_instance.similarity_search_with_score.return_value = [
                    (Document(page_content="Vector result", metadata={"source": "vec.txt"}), 0.1)
                ]
                mock_langchain_chroma.return_value = mock_lc_instance
...
class TestVectorStoreManager:
    ...
    async def test_search_all_stores(self):
        ...
        mock_store1.search = AsyncMock(return_value=[
            SearchResult(
                document=Document(page_content="Result 1"),
                score=0.9,
                metadata={}
            )
        ])
        ...
        mock_store2.search = AsyncMock(return_value=[
            SearchResult(
                document=Document(page_content="Result 2"),
                score=0.8,
                metadata={}
            )
        ])
    ...
    async def test_optimize_stores(self):
        ...
        assert results["store1"] == True
        assert results["store2"] == True
...
async def test_vector_store_integration():
    ...
    with patch('chromadb.Client') as mock_client:
        ...
        with patch('langchain.vectorstores.Chroma') as mock_langchain_chroma:
            ...
            mock_lc_instance.similarity_search_with_score.return_value = [
                (Document(page_content="Test result", metadata={"source": "test.txt"}), 0.1)
            ]
            mock_langchain_chroma.return_value = mock_lc_instance
