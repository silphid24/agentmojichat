...
async def test_optimized_hybrid():
    ...
    try:
        ...
        for query in test_queries:
            ...
            improvement = ((basic_time - hybrid_time) / basic_time) * 100 if basic_time > 0 else 0
            print(f"   ⚡ 성능 차이: {improvement:+.1f}%")
            
            # 검색 품질 비교
            if hybrid_meta.get('result_details'):
                avg_score = sum(d.get('combined_score', 0) for d in hybrid_meta['result_details']) / len(hybrid_meta['result_details'])
