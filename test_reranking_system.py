...
async def test_reranking_system():
    ...
    try:
        ...
        for i, test_case in enumerate(test_queries, 1):
            ...
            try:
                no_rerank_docs, no_rerank_meta = await hybrid_pipeline.search_with_hybrid(
            ...
            try:
                ...
                print(f"      🔎 검색 타입: {rerank_meta.get('search_type', 'unknown')}")
                ...
                if rerank_meta.get('result_details'):
                    ...
                    for j, detail in enumerate(rerank_meta['result_details'][:3], 1):
                        ...
                        final_score = breakdown.get('final_score', breakdown.get('combined_score', 0))
                        ...
                        print(f"         {j}. {source} (점수: {final_score:.4f}){change_indicator}")
                        if rerank_score > 0:
                    ...
                    avg_rank_change = sum(abs(change) for change in rank_changes) / len(rank_changes)
                    
                    print(f"      📈 순위 변화: 상승 {positive_changes}개, 하락 {negative_changes}개")
                    print(f"      📊 평균 순위 변화: {avg_rank_change:.1f}")
                    
                    total_rank_changes.extend(rank_changes)
                    
                    # 개선 여부 판단
                    improvement_detected = avg_rank_change > 0.5 or positive_changes > negative_changes
            ...
            # 성능 비교
            performance_overhead = ((rerank_time - no_rerank_time) / no_rerank_time) * 100 if no_rerank_time > 0 else 0
            print(f"   ⏱️  성능 오버헤드: +{performance_overhead:.1f}% ({rerank_time:.3f}초 vs {no_rerank_time:.3f}초)")
        ...
        print(f"   📊 전체 개선율: {(total_improvement_count / len(test_queries)) * 100:.1f}%")
        ...
        if total_rank_changes:
            ...
            avg_total_change = sum(abs(change) for change in total_rank_changes) / len(total_rank_changes)
...
async def test_reranker_models():
    ...
    try:
        ...
        # 기본 검색으로 문서 가져오기
        docs_with_scores = rag_pipeline.vectorstore.similarity_search_with_score(test_query, k=5)
        docs = [doc for doc, _ in docs_with_scores]
        original_scores = [1.0 / (1.0 + score) for _, score in docs_with_scores]  # 거리를 유사도로 변환
            ...
            try:
                ...
                if results:
                    ...
                    for i, result in enumerate(results[:3], 1):
                        ...
                        print(f"      {i}. {source}")
                        print(f"         원본: {result.original_score:.3f}, 리랭크: {result.rerank_score:.3f}")
                        print(f"         최종: {result.combined_score:.3f}{change_indicator}")
                
                # 순위 변화 통계
                rank_changes = [r.rank_change for r in results]
                avg_change = sum(abs(change) for change in rank_changes) / len(rank_changes) if rank_changes else 0
