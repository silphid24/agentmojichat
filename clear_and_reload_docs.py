...
async def clear_vector_store():
    ...
    try:
        ...
        if rag_pipeline.vectordb_dir.exists():
            ...
            print(f"   ✅ 삭제됨: {rag_pipeline.vectordb_dir}")
        else:
            print(f"   ℹ️  이미 비어있음: {rag_pipeline.vectordb_dir}")
        
        # Recreate directory
        rag_pipeline.vectordb_dir.mkdir(parents=True, exist_ok=True)
        print(f"   ✅ 디렉토리 재생성: {rag_pipeline.vectordb_dir}")
...
async def verify_indexing():
    ...
    try:
        ...
        if result and result.get('answer'):
            ...
            print(f"   ⚠️  쿼리 응답 없음 (문서가 없거나 관련성 낮음)")
...
async def main():
    ...
    try:
        ...
        print(f"\n📍 1단계: 벡터 스토어 초기화")
        if not await clear_vector_store():
            print("❌ 벡터 스토어 초기화 실패")
            return
