...
from app.core.model_optimization import get_optimized_reranker_model, adaptive_selector
...
class HybridSearchEngine:
    ...
    def _initialize_corpus(self):
        """BM25를 위한 코퍼스 초기화"""
        try:
            # Base pipeline의 vectorstore 초기화 강제 실행
            if hasattr(self.vectorstore, '_initialize_vectorstore'):
                logger.info("Initializing base vectorstore for hybrid search...")
                self.vectorstore._initialize_vectorstore()
            elif hasattr(self.vectorstore, 'base_pipeline') and hasattr(self.vectorstore.base_pipeline, '_initialize_vectorstore'):
                logger.info("Initializing base pipeline vectorstore...")
                self.vectorstore.base_pipeline._initialize_vectorstore()
                self.vectorstore = self.vectorstore.base_pipeline.vectorstore
            
            # Enhanced RAG pipeline에서 vectorstore 가져오기 (지연 초기화 대응)
            from app.rag.enhanced_rag import rag_pipeline
            if hasattr(rag_pipeline, '_initialize_vectorstore'):
                logger.info("Forcing vectorstore initialization from enhanced RAG pipeline...")
                rag_pipeline._initialize_vectorstore()
                if rag_pipeline.vectorstore:
                    self.vectorstore = rag_pipeline.vectorstore
                    logger.info("Successfully updated vectorstore from enhanced RAG pipeline")

            # ChromaDB 벡터스토어 초기화 확인 및 강제 초기화
            if not hasattr(self.vectorstore, '_collection') or self.vectorstore._collection is None:
                logger.warning("Vector store collection is not initialized. Attempting to force initialization...")
                
                # 더미 검색을 통해 지연 초기화 트리거
                try:
                    logger.info("Triggering lazy initialization with dummy search...")
                    _ = self.vectorstore.similarity_search("dummy", k=1)
                    logger.info("Lazy initialization successful via dummy search")
                except Exception as dummy_error:
                    logger.warning(f"Dummy search failed: {dummy_error}")
                
                # ChromaDB 컬렉션 직접 초기화 시도
                if (not hasattr(self.vectorstore, '_collection') or self.vectorstore._collection is None) and hasattr(self.vectorstore, '_client') and hasattr(self.vectorstore, '_collection_name'):
                    try:
                        self.vectorstore._collection = self.vectorstore._client.get_collection(
                            name=self.vectorstore._collection_name
                        )
                        logger.info(f"Successfully initialized collection: {self.vectorstore._collection_name}")
                    except Exception as init_error:
                        logger.error(f"Failed to get existing collection: {init_error}")
                        # 컬렉션이 없으면 생성 시도
                        try:
                            self.vectorstore._collection = self.vectorstore._client.create_collection(
                                name=self.vectorstore._collection_name
                            )
                            logger.info(f"Created new collection: {self.vectorstore._collection_name}")
                ...
                # 여전히 None이면 에러
                if not hasattr(self.vectorstore, '_collection') or self.vectorstore._collection is None:
                    raise ValueError("Vector store collection could not be initialized")
            ...
            if results and "documents" in results and results["documents"]:
                ...
                logger.info(
                    f"Hybrid search initialized with {len(self.doc_texts)} documents"
                )
            else:
                logger.warning("No documents found in vector store. You may need to load documents first.")
                logger.info("To load documents, run: python upload_docs.py")
    ...
    def search(
        self, query: str, k: int = 5, score_threshold: float = 0.05  # Lowered threshold for better recall
    ) -> List[Tuple[Document, float, Dict[str, float]]]:
        """하이브리드 검색 실행"""
        try:
            # 1. 벡터 검색 (더 많은 후보 검색)
            vector_results = self._vector_search(query, k=min(100, max(20, len(self.documents))))
            ...
            for doc, vector_score in vector_results:
                ...
                for i, corpus_doc in enumerate(self.documents):
                    if (
                        corpus_doc.page_content == doc.page_content
                        and corpus_doc.metadata.get("chunk_id")
...
class HybridRAGPipeline:
    ...
    def __init__(self, enhanced_rag_pipeline):
        ...
        self.hybrid_engine = HybridSearchEngine(
            ...
            vector_weight=0.4,  # 벡터 검색 가중치 (0.5 → 0.4)
            keyword_weight=0.4,  # 키워드 검색 가중치 (0.3 → 0.4)
            bm25_weight=0.2,  # BM25 검색 가중치 유지
        )
    ...
    async def search_with_hybrid(
        ...
        query: str,
        k: int = 5,
        score_threshold: float = 0.05,  # Lowered for better recall
        use_query_rewriting: bool = True,  # Enable query rewriting for better quality
        ...
        try:
            ...
            # 하이브리드 검색 실행 (병렬 처리)
            if len(queries) > 1:
                all_results = await self._parallel_hybrid_search(
                    queries, k, score_threshold
                )
            else:
                # 단일 쿼리는 직접 처리 (더 많은 초기 결과 검색)
                results = self.hybrid_engine.search(
                    queries[0], k=k * 4, score_threshold=score_threshold
                )
                all_results = [
                    (doc, score, score_details, queries[0])
            ...
            if use_reranking and all_results:
                ...
                # 리랭킹을 위한 데이터 준비
                docs_for_rerank = [
                    result[0] for result in all_results[: k * 2]
                ]  # 더 많은 후보로 리랭킹
            ...
            logger.error(f"Hybrid search error: {e}")
            # 폴백: 기존 벡터 검색 사용
            return await self.base_pipeline.search_with_rewriting(
    ...
    async def answer_with_hybrid_search(
        ...
        try:
            ...
            if use_query_rewriting is None:
                ...
                logger.info(
                    f"Adaptive hybrid config: rewrite={use_query_rewriting}, rerank={use_reranking}"
                )
            else:
            ...
            if cached_result:
                logger.info(
                    f"Hybrid cache hit: {query[:50]}... ({time.time() - start_time:.3f}s)"
                )
...
답변:""",
            ...
            confidence = self._calculate_confidence(
                answer_text, documents, search_metadata
            )
            ...
            logger.info(
                f"Hybrid query processed and cached: {query[:50]}... ({time.time() - start_time:.3f}s)"
            )
    ...
    def _calculate_confidence(
        self, answer: str, documents: List[Document], search_metadata: Dict[str, Any]
    ) -> str:
        """고도화된 신뢰도 계산"""
        try:
    ...
    async def _parallel_hybrid_search(
        ...
        try:
            ...
            logger.info(
                f"Parallel hybrid search completed in {elapsed_time:.3f}s for {len(queries)} queries"
            )
            ...
            for i, results in enumerate(results_per_query):
                if isinstance(results, Exception):
                    logger.error(
                        f"Hybrid search failed for query '{queries[i]}': {results}"
                    )
                    continue
            ...
            for query in queries:
                try:
                    ...
                    logger.error(
                        f"Fallback search failed for query '{query}': {query_error}"
                    )
    ...
    async def _single_hybrid_search(
        ...
        try:
            ...
            results = await loop.run_in_executor(
                None,
                lambda: self.hybrid_engine.search(
