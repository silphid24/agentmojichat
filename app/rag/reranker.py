...
class CrossEncoderReranker:
        ...
        try:
            if self.use_local_model:
                ...
                try:
                    ...
                    if not model_loaded:
                        logger.warning("All cross-encoder models failed, using fallback similarity")
                        self.model = None
    ...
    def rerank(
        ...
        try:
            ...
            for new_rank, result in enumerate(results):
                ...
                result.rank_change = original_rank - new_rank  # 양수면 상승, 음수면 하락
            ...
            logger.info(f"Reranking completed in {elapsed_time:.3f}s for {len(documents)} documents")
    ...
    def _compute_rerank_scores(self, query: str, documents: List[Document]) -> List[float]:
        """리랭킹 점수 계산"""
    ...
    def _compute_cross_encoder_scores(self, query: str, documents: List[Document]) -> List[float]:
        """교차 인코더를 사용한 점수 계산"""
        try:
            # 쿼리-문서 쌍 생성
    ...
    def _compute_fallback_scores(self, query: str, documents: List[Document]) -> List[float]:
        """폴백 점수 계산 (텍스트 유사도)"""
        scores = []
    ...
    def advanced_rerank(
        ...
        try:
            ...
            for result in base_results:
                ...
                keyword_density = sum(1 for word in doc_words if word in query_words) / max(len(doc_words), 1)
            ...
            logger.info(f"Advanced reranking completed for {len(enhanced_results)} documents")
