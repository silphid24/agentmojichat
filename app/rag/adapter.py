...
class RAGPipelineAdapter:
    ...
    async def query(self, query: RAGQuery) -> RAGResponse:
        ...
        try:
            ...
            result = await self.enhanced_pipeline.answer_with_confidence(
                query=query.query,
                k=query.top_k or 5,
                score_threshold=query.score_threshold or 1.6  # Adjusted for ChromaDB distance
            )
    ...
    def clear_index(self) -> None:
        ...
        try:
            ...
            self.enhanced_pipeline.vectorstore = self.enhanced_pipeline._create_vectorstore()
    ...
    async def update_chunk_size(self, chunk_size: int, chunk_overlap: int) -> None:
        ...
        logger.info(f"Updated chunk configuration: size={chunk_size}, overlap={chunk_overlap}")
