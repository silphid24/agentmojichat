...
class KakaoTalkAdapter(BaseAdapter):
    ...
    async def _create_message_template(self, message: PlatformMessage) -> Dict[str, Any]:
    ...
    async def _create_text_template_with_buttons(self, message: PlatformMessage) -> Dict[str, Any]:
        ...
        template = {
            ...
            "button_title": message.buttons[0].text[:14] if message.buttons else "더보기",
        }
        
        return template
    
    async def _convert_attachments(self, kakao_attachments: Dict[str, Any]) -> List[Attachment]:
