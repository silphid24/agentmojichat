...
async def create_chat_completion(
    request: ChatRequest, current_user: Annotated[UserInDB, Depends(get_current_user)]
):
    ...
    if not state:
        state = state_manager.create_state(
            session_id=session_id, agent_id="chat_agent", user_id=str(current_user.id)
        )

    # Process with agent
    try:
        ai_message = await agent_manager.process_with_agent(
            lc_messages, provider=request.provider, model=request.model
        )
