...
async def create_store(
    request: CreateStoreRequest, current_user: str = Depends(get_current_user)
):
...
@router.get("/stores")
async def list_stores(current_user: str = Depends(get_current_user)):
...
async def delete_store(store_id: str, current_user: str = Depends(get_current_user)):
    ...
    try:
        ...
        if request.use_hybrid and hasattr(store, "hybrid_search"):
            ...
            results = await store.search(
                query=request.query, k=request.k, filter=request.filter
            )
...
async def add_documents(
    request: AddDocumentsRequest, current_user: str = Depends(get_current_user)
):
...
async def delete_documents(
    request: DeleteRequest, current_user: str = Depends(get_current_user)
):
    """Delete documents from a vector store"""
    try:
        store = vector_store_manager.get_store(request.store_id)

        success = await store.delete(ids=request.ids, filter=request.filter)
...
async def clear_store(store_id: str, current_user: str = Depends(get_current_user)):
...
async def optimize_stores(current_user: str = Depends(get_current_user)):
    """Optimize all vector stores"""
    try:
        results = await vector_store_manager.optimize_stores()

        return {"message": "Optimization completed", "results": results}
...
async def migrate_store_data(
    from_store_id: str, to_store_id: str, current_user: str = Depends(get_current_user)
):
