...
class VectorStoreManager:
    ...
    async def add_documents_to_all(
        self, documents: List[Any], exclude_stores: Optional[List[str]] = None
    ) -> Dict[str, List[str]]:
    ...
    def remove_store(self, store_id: str) -> bool:
        ...
        if store_id in self.stores:
            ...
            if self.default_store == store_id:
                self.default_store = (
                    list(self.stores.keys())[0] if self.stores else None
                )
    ...
    async def migrate_data(
        self, from_store_id: str, to_store_id: str, batch_size: int = 100
    ) -> bool:
