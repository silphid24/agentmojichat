...
class AsyncBatchProcessor:
    ...
    async def process_batch(
        ...
        # 병렬 처리
        if is_async:
            tasks = [self._process_single_batch_async(batch, processor) for batch in batches]
            batch_results = await asyncio.gather(*tasks, return_exceptions=True)
        else:
            tasks = [self._process_single_batch_sync(batch, processor) for batch in batches]
            batch_results = await asyncio.gather(*tasks, return_exceptions=True)
    ...
    async def _process_single_batch_async(self, batch: List[T], processor: Callable) -> Any:
        """단일 배치 비동기 처리"""
    ...
    async def _process_single_batch_sync(self, batch: List[T], processor: Callable) -> Any:
        """단일 배치 동기 처리 (스레드풀 사용)"""
        async with self.semaphore:
            try:
        ...
        async with self.semaphore:
            try:
                if asyncio.iscoroutinefunction(search_func):
                    ...
                    loop = asyncio.get_event_loop()
                    result = await loop.run_in_executor(None, search_func, query, **search_params)
...
async def run_parallel_searches(
    ...
    """병렬 검색 편의 함수"""
    return await parallel_search_manager.parallel_search(queries, search_functions, search_params)
