...
from app.core.model_optimization import (
    warm_up_all_models, optimize_model_performance,
    model_manager, initialize_model_configurations
)
...
class StartupManager:
    ...
    async def initialize_application(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"   Tasks completed: {len(initialization_results['tasks_completed'])}")
            logger.info(f"   Tasks failed: {len(initialization_results['tasks_failed'])}")
    ...
    async def _initialize_cache_system(self, results: Dict[str, Any]):
        ...
        try:
            ...
            if cache_result.get("success", False):
                ...
                logger.info(f"   Cache type: {cache_result.get('cache_type', 'unknown')}")
                
                results["tasks_completed"].append("cache_system")
                results["details"]["cache_system"] = f"✅ {cache_result.get('cache_type')} ({elapsed_time:.3f}s)"
            else:
    ...
    async def _initialize_model_system(self, results: Dict[str, Any]):
        ...
        try:
            ...
            model_count = len([m for m in model_stats.keys() if model_manager.warm_up_status.get(m, False)])
            ...
            results["details"]["model_system"] = f"✅ {model_count} models warmed up ({elapsed_time:.3f}s)"
    ...
    async def _apply_performance_optimizations(self, results: Dict[str, Any]):
        ...
        try:
            ...
            results["details"]["performance_optimization"] = f"✅ Completed ({elapsed_time:.3f}s)"
    ...
    async def _validate_system_health(self, results: Dict[str, Any]):
        ...
        try:
            ...
            results["details"]["health_validation"] = f"✅ {healthy_systems}/{total_systems} systems healthy"
