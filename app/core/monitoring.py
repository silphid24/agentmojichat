...
class PerformanceMonitor:
        ...
        self.thresholds.update(
            {
                ...
                "cache_hit_rate": PerformanceThreshold(
                    warning_threshold=0.50,  # 50% (낮으면 경고) - 완화된 임계값
                    error_threshold=0.30,  # 30%
                    critical_threshold=0.10,  # 10% - 매우 낮을 때만 심각
                ),
    ...
    def record_request_end(
        ...
        try:
            ...
            start_metrics = [
                m
                for m in self.collector.metrics[f"request_start_{request_id}"]
                if m.timestamp
            ]
            ...
            if success:
                self.collector.record_counter(
                    "requests_success", labels={"operation": operation}
                )
            else:
                self.collector.record_counter(
                    "requests_error", labels={"operation": operation}
                )
            ...
            if additional_metrics:
                for metric_name, value in additional_metrics.items():
                    self.collector.record_gauge(
                        metric_name, value, labels={"operation": operation}
                    )
    ...
    def record_model_performance(
        self, model_name: str, operation: str, duration: float, success: bool = True
    ):
        """모델 성능 기록"""
    ...
    def _calculate_and_check_error_rate(self, operation: str):
        ...
        try:
            success_count = len(
                [
                    m
                    for m in self.collector.metrics["requests_success"]
                    if m.labels.get("operation") == operation
                    and m.timestamp > time.time() - 300  # 5분 윈도우
                ]
            )
            ...
            error_count = len(
                [
                    m
                    for m in self.collector.metrics["requests_error"]
                    if m.labels.get("operation") == operation
    ...
    def _calculate_and_check_cache_hit_rate(self, cache_type: str):
        ...
        try:
            hits = len(
                [
                    m
                    for m in self.collector.metrics["cache_hits"]
                    if m.labels.get("type") == cache_type
                    and m.timestamp > time.time() - 300  # 5분 윈도우
                ]
            )
            ...
            misses = len(
                [
                    m
                    for m in self.collector.metrics["cache_misses"]
                    if m.labels.get("type") == cache_type
                    and m.timestamp > time.time() - 300
                ]
            )
            ...
            if total_requests > 0:
                ...
                if (
                    hit_rate
                    < self.thresholds.get(
                        "cache_hit_rate", PerformanceThreshold(0.8, 0.6, 0.4)
                    ).critical_threshold
                ):
                    self._create_alert(
                        "cache_hit_rate",
    ...
    def _check_thresholds(self, metric_name: str, value: float):
        ...
        if value >= threshold.critical_threshold:
            self._create_alert(
                ...
                AlertLevel.CRITICAL,
                value,
                f"{metric_name} critically high",
            )
        elif value >= threshold.error_threshold:
            self._create_alert(
                metric_name, AlertLevel.ERROR, value, f"{metric_name} high"
            )
        elif value >= threshold.warning_threshold:
            self._create_alert(
                metric_name, AlertLevel.WARNING, value, f"{metric_name} elevated"
            )

    def _create_alert(
        self, metric_name: str, level: AlertLevel, value: float, message: str
    ):
        """알림 생성"""
        alert = Alert(
            level=level,
            ...
            value=value,
            threshold=getattr(
                self.thresholds.get(metric_name), f"{level.value}_threshold", 0
            ),
            timestamp=time.time(),
        )
        ...
        log_level(
            f"ALERT [{level.value.upper()}] {metric_name}: {message} (value: {value:.3f})"
        )
    ...
    def _get_system_status(self) -> str:
        ...
        try:
            ...
            recent_critical = [
                alert
                for alert in self.alerts
                if alert.timestamp > time.time() - 300
            ...
            recent_errors = [
                ...
                for alert in self.alerts
                if alert.timestamp > time.time() - 300
                and alert.level == AlertLevel.ERROR
                and not alert.resolved
            ]
            ...
            recent_warnings = [
                ...
                for alert in self.alerts
                if alert.timestamp > time.time() - 300
                and alert.level == AlertLevel.WARNING
                and not alert.resolved
            ]
            ...
            with timer(name, labels):
                try:
                    ...
                    performance_monitor.collector.record_counter(
                        f"{func.__name__}_success", labels=labels
                    )
                    return result
                except Exception:
                    performance_monitor.collector.record_counter(
                        f"{func.__name__}_error", labels=labels
                    )
                    raise
