...
class AdaptiveFeatureManager:
    ...
    def analyze_query(
        ...
        try:
            ...
            logger.info(
                f"Query analysis: {complexity.value} complexity, {keyword_count} keywords, mode: {recommended_mode.value}"
            )
            return analysis
    ...
    def _estimate_processing_time(
        self, complexity: QueryComplexity, keyword_count: int
    ) -> float:
        """처리 시간 추정"""
        base_times = {
    ...
    def _decide_features(
        ...
        if complexity == QueryComplexity.SIMPLE:
            features.update(
                {"query_rewriting": False, "parallel_search": False, "reranking": False}
            )
            ...
            features.update(
                {"query_rewriting": True, "parallel_search": True, "reranking": True}
            )

        # 모드별 조정
        if mode == ProcessingMode.FAST:
            features.update({"query_rewriting": False, "reranking": False})
...
def should_enable_feature(
    query: str, feature_name: str, context: Optional[Dict[str, Any]] = None
) -> bool:
    """특정 기능 활성화 여부 판단"""
...
def get_optimal_search_config(
    ...
    return {
        "use_query_rewriting": analysis.features_to_enable.get(
            "query_rewriting", False
        ),
        "use_parallel_search": analysis.features_to_enable.get(
            "parallel_search", False
        ),
